...
usage() {
  echo "Usage: $(basename "$0") --sentinel-master <name> --sentinel-port <port> --redis-port <port> --auth <password> [--timeout <seconds>]"
  echo ""
  echo "  --sentinel-master  Sentinel master name (e.g. mymaster)"
  echo "  --sentinel-port    Sentinel port (default: 26379)"
  echo "  --redis-port       Redis port (default: 6379)"
  echo "  --auth             Redis/Sentinel password"
...
[[ -z "${SENTINEL_MASTER}" ]] && echo "ERROR: --sentinel-master is required" >&2 && usage
